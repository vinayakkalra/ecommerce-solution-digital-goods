<html>
    <head>

    </head>
    <body>
        <!-- Top Bar- Start -->
<div class="container-fluid" style="background-color:#24436e; height: auto;">
    <div class="container-sm" style="margin: auto;">
        <div class="row" style="color:white; text-align: center; padding: 2px; font-weight: bold;">
            <div class="col-sm" class="text font-Lato-Regular">
                SAVE 25% COUPON : <u class="text font-Lato-Regular">FLAT25</u>
            </div>
        </div>
    </div>
</div>
<!-- Top Bar- End -->

<!-- Second Bar-Start -->
<div class="container-fluid" style=" height: auto; padding:0;">
    <div class="container" style=" width: 100%;">
        <div class="row row-cols-3 align-items-center" style="padding: 2%;">
            <div class="col-sm">
                <!-- Hamburger Menu-Start -->
                <div id="mySidenav" class="sidenav">
                    <a href="javascript:void(0)" class="closebtn" onclick="closeNav()">&times;</a>
                    <a href="#" class="text font-Lato-Regular" style="text-align: left;">HOME</a>
                    <a href="#" class="text font-Lato-Regular" style="text-align: left;">B2B & B2C</a>
                    <a href="#" class="text font-Lato-Regular" style="text-align: left;">GOVERNMENT</a>
                    <a href="#" class="text font-Lato-Regular" style="text-align: left;">STUDENTS</a>
                    <a href="#" class="text font-Lato-Regular" style="text-align: left;">JOB SEEKERS</a>
                    <a href="#" class="text font-Lato-Regular" style="text-align: left;">ALL INDIA</a>
                    <a href="#" class="text font-Lato-Regular" style="text-align: left;">OTHER DATABASES</a>
                    <a href="#" class="text font-Lato-Regular" style="text-align: left;">ABOUT US</a>
                    <a href="#" class="text font-Lato-Regular" style="text-align: left;">CONTACT US</a>
                    <a href="#" class="text font-Lato-Regular" style="text-align: left;">SHOP</a>
                    <a href="#" class="text font-Lato-Regular" style="text-align: left;">LOGIN</a>
                  </div>
                  
                  <!-- Use any element to open the sidenav -->
                  <span onclick="openNav()">&#9776;</span>
                  
                  <!-- Add all page content inside this div if you want the side nav to push page content to the right (not used if you only want the sidenav to sit on top of the page -->
                <!-- Hamburger Menu-End -->
            </div>
            <div class="col-sm">
                <img src="./img/logo.png" width="100%" height="100%">
            </div>
            <div class="col-sm text-center" >
                <img src="./img/mob.PNG" style="height: 30%; width: 30%;">
            </div>
        </div>
    </div>
</div>
<!-- Second Bar End -->
        
    </body>
</html>
